...
Purpose
John the Ripper
CLI password-cracking tool used to run dictionary/brute-force attacks against extracted hashes
Johnny
GUI front-end for John the Ripper, used to manage attacks and view cracked results
Online HashCrack — PDF Hash Extractor
Web tool used to extract a pdf2john/hashcat-compatible hash from a locked PDF
Networkwalks Hash Calculator
Browser-based tool to generate/extract hashes (MD5, SHA family, and PDF hashes) locally
Networkwalks Password Cracker
Web-based cracking tool used to test candidate passwords against the extracted hash
Methodology
Hash Extraction — Uploaded the locked PDF to a hash extraction tool to generate a $pdf$... formatted hash (pdf2john/hashcat-compatible format).
Attack Setup — Loaded the extracted hash into John the Ripper via the Johnny GUI, selecting the PDF format for the attack.
Cracking — Ran a dictionary-based attack against the hash. The Networkwalks Password Cracker tool was also used to test candidate passwords against the same hash for comparison.
Verification — Successfully cracked passwords were used to unlock the original PDF files, confirming correct recovery.
Results
